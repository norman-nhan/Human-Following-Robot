...
class Go2Goal(State):
    ...
    def camera_info_callback(self, msg: CameraInfo):
        if not self.camera_info_received:
            ...
            self.get_logger().info(f"Camera Info received: {self.camera_mat, self.dist_coeffs}")
    ...
    def detect_and_navigate(self):
        ...
        if ids is not None:
            ...
            for i, marker_id in enumerate(ids.flatten()):
                ...
                self.get_logger().info(f"Detected marker {marker_id}, navigating to offset pose")
                ...
                try:
                    ...
                    transform = self.tf_buffer.lookup_transform(
                        "map", pose_cam.header.frame_id, rclpy.time.Time()) # make sure that there is camera's frame in robot's TF tree! Otherwise, this won't work.
                    pose_map = do_transform_pose(pose_cam, transform)

                    self.get_logger().info(f"Transformed pose: x={pose_map.pose.position.x:.2f}, y={pose_map.pose.position.y:.2f}, yaw={yaw:.2f}")
    ...
    def goToPose(self, x: float, y: float, yaw: float):
        ...
        self.get_logger().debug("Waiting for NavigateToPose server...")
